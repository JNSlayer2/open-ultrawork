...
- `scripts/flow-levels.mjs`：S/M/L/XL 等級化模板——budget 硬上限、fan-out 上限、loop-until-dry、L 級以上 adversarial verify、XL 需 `UW_XL_AUTHORIZED=1` 明確授權。fan-out 一律 schema 短輸出（強、省、命中核心）。用法：`UW_LEVEL=M UW_TASK="..." node scripts/flow-levels.mjs`。
- **copilot lane 跨品牌 fallback**：codex backend 遇 auth 類錯誤（401 / token invalidated / not authenticated）時，`agent()` 自動降級到 `gateway:grok-build` 一次（log 標 `→fb:`），避免單一 vendor 的壞 session 報廢整個工作流。`UW_AUTH_FALLBACK_MODEL` 可覆寫，設 `off` 停用。
- **孤兒 auth.json failure mode**：subagent 專用 CODEX_HOME 若留著舊 `auth.json` 副本，token 輪換後永遠 401，且搶刷可能弄壞主 session。對策：子 home 的 auth.json 一律 symlink 指向主 auth.json。
- **economy 片段審碼會高信度誤報**：便宜模型對程式碼片段給出的 P0 可能正是 spec 的刻意行為；fan-out findings 必須由主控對全檔/spec 逐條驗證後才可進報告。

## 更新（Update）

已 clone 的 checkout 一鍵更新（pull + 免額度自檢，exit 0 = 成功）：
