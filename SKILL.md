...
執行：`node my-workflow.mjs`（Codex CLI 直接以 shell 跑）。Hooks：`agent(prompt,{tier|backend,model,schema,retries,label})`、`parallel`、`pipeline`、`verify(claim,{tiers,threshold})`、`setBudget(usd)`、`costReport()`、`log`。
...
- **ChatGPT Pro Consult / GPT heavy**：主控、落地規劃、工具執行語義、跨步驟決策；適合當 `primary`，不適合做大量廉價枚舉。
