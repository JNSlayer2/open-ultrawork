...
7. **ChatGPT Pro Research MCP lane**：若人類有 ChatGPT Pro / Deep Research，可把長篇外部研究分流到產品內訂閱額度；優先走 bounded `chatgpt-pro-mcp` async UI bridge（`ProResearchJobV1` → submit → reattach/fetch → `ProResearchResultV1`），手動貼回只作為 fallback。這不是 API fan-out，也不是同步 dropdown model。
...
- **ChatGPT lanes**：日常協作優先走 Codex App / `model_gateway` 的 `gpt-5.5` 同 thread fast consult；舊 `chatgpt-pro-consult` 只作 hidden/deprecated alias，不宣稱等同 ChatGPT App Deep Research。真正 Pro Research 走 `ProResearchJobV1`：Codex/open-ultrawork 產生 bounded job，`chatgpt-pro-mcp` 可用 async Web UI bridge 送入 ChatGPT Pro，subagent 以 `fetch_deep_research_result({ wait_seconds })` 重新接回並匯入來源與 claim ledger；手動貼回只作 fallback。
- **起手式**：日常小修、明確 bugfix、跑測試與落地執行，預設留在 Codex executor 模型；架構規劃、研究級審查、多模型分工、高風險決策或 claim/evidence 協作，先用 `gpt-5.5` fast consult 或 `chatgpt-pro-mcp` academic lane；定案後再回 Codex executor 落地。不要把 Pro 當 bulk subagent，也不要把 executor 任務長期鎖在 Pro。
...
`RPO` 在本 skill 中指 **Research / Plan / Operate**：`gpt-5.5` / hidden `chatgpt-pro-consult` 負責同 thread 快速規劃／批評；真正 ChatGPT Pro research 走 bounded async research job；Codex / OpenClaw / sandbox 宿主負責可驗證執行。若工作從 Codex 開始，主線是 **Codex → GPT fast consult 或 `chatgpt-pro-mcp` research lane**；若工作從 ChatGPT 開始，才使用 **ChatGPT Pro → Codex MCP Hub** 前門。不要把 ChatGPT Web 偽裝成同步 `/v1/responses` dropdown model；Web/App UI bridge 只能是有狀態、有護欄的 async research lane。

### 兩面打通

1. **Codex → GPT fast consult（Codex 主控，日常主線）**
   - `model_gateway` catalog 應以 `gpt-5.5` 作為日常 GPT planning / critique / bounded review 入口；`chatgpt-pro-consult` / `chatgpt-pro` 只作 hidden/deprecated compatibility alias。
   - 兼容 alias 只把 request body 的 `model` 改寫成上游 `gpt-5.5`，其餘 Codex session headers、thread/tool context、MCP tool results 原封 passthrough。
   - `copilot` / `T1-copilot` tier 可指向 `codex:gpt-5.5` 或兼容 alias；它是 Codex-native fast consult，`pro_research_equivalence=false`。
   - 健檢以 `gpt-5.5 -> Claude/Grok/MiniMax -> gpt-5.5` same-thread smoke 驗證上下文不斷層。
...
3. **ChatGPT Pro / Deep Research async research bridge**
   - 需要長篇外部研究時，Codex/open-ultrawork 產生 `ProResearchJobV1` / bounded prompt，優先交給 `chatgpt-pro-mcp`。
   - `submit_deep_research` 可在 `CHATGPT_PRO_MCP_RETURN_AFTER_SUBMIT=1` 下快速返回 `running`；subagent 後續用 `fetch_deep_research_result({ task_id, wait_seconds })` reattach 同一 ChatGPT thread，匯入 `ProResearchResultV1`。
   - `authority.author_model` 應標 `chatgpt-pro-web` 或更精確的已驗證模式；若 `authority.research_mode.enabled=false`，只能稱為 ChatGPT Pro Web research memo，不可宣稱 confirmed Deep Research。
   - 手動在 ChatGPT Pro / Deep Research 執行並貼回 Markdown/PDF/source links 仍是 fallback；Codex 只採納經 primary-source cross-check 的 claims。
...
- `proAcademicReview()` 若走 `gpt-5.5` / hidden `chatgpt-pro-consult`，只能算 Codex-native fast schema critique；真正 Pro research reviewer 走 `chatgpt-pro-mcp` async job。無論哪條 lane，若沒有 `tool_intent_bridge` / `sandbox_executor`，只能輸出研究與 patch intent，不能聲稱已寫檔或跑 shell。
...
- 若使用 `chatgpt-pro-mcp`，最低驗收是：guarded submit 會 fail-closed、real async smoke 可 `running -> fetch -> completed`、長任務可等待數分鐘以上、`marker_found=true`、source links 可匯入、所有 claims 預設 `needs_verification`。
...
硬邊界：不把 secrets、私有帳務、完整 logs、私人 thread id、原始 prompt dump、token、本機絕對路徑、browser profile、cookies 或 raw screenshots 放進 ChatGPT Pro / MCP public connector；`chatgpt-pro-mcp` 不暴露 raw browser tools，只能送 bounded research job、poll/fetch、回收 report。ChatGPT Pro 預設只做研究、規劃、patch intent 或 handoff。任何寫檔、shell、部署、下單或風險參數修改都必須由本地授權的 `executor_host` 執行並留下 trace。
...
**為何能贏過官方單廠牌 Workflow**：官方全 Claude，每個 subagent 都燒 premium quota；本編排器**成本分層**——bulk fan-out 走近免費的 MiniMax M3 / gpt-5.4-mini，premium Opus/Fable/GPT fast consult/Pro Research MCP 只留給收斂與裁決，且對抗驗證用**不同廠牌**模型（去相關盲點）。實測同樣 fan-out 成本約 1/100。
...
- `consultant` / `copilot` / `T1-copilot` → codex `gpt-5.5`（Codex-native GPT fast critique/assistant，同 thread passthrough；`chatgpt-pro-consult` 只作 hidden/deprecated alias）
...
後端（皆獨立 context，本機已實測）：`claude`（claude -p，JSON、無 session、工具全關）、`codex`（codex exec、`-c mcp_servers={}`、`-o` 乾淨輸出、預設只在 `~/.codex-sub` 已明確設定 `model_provider = "model_gateway"` 時使用 sub-home，否則回落 `CODEX_HOME=$HOME/.codex`；可用 `UW_CODEX_HOME` 覆寫；避免把 Codex home 放在 noowners 外接卷）、`gateway`（POST `/v1/responses` stream:false 取 `output_text`）。
...
const dr = deepResearchPlanForTask(task); // ProResearchJobV1 prompt/import plan; confirmed Deep Research only when MCP reports research_mode.enabled=true
const packet = academicContinuityPacket({ task, claims, verificationCommands, verificationReceipts, artifactRefs });
const proPrompt = proAcademicPrompt(packet); // claim/evidence/rebuttal prompt for GPT fast critique or chatgpt-pro-mcp research lane
...
- `budgetPlan()` 會把 `chatgpt_pro_research_mcp` 標成 subscription/UI lane / `apiFanoutExcludes`；它不消耗 API fan-out 預算，但仍要受人類產品內額度、UI 狀態與任務停止條件約束。
...
- `academicContinuityPacket()` / `proAcademicPrompt()` / `proAcademicReview()` 是 Pro 級學術協作入口：所有重要結論先變成 claim/evidence ledger，再讓 GPT fast critique 或 `chatgpt-pro-mcp` research lane 做反駁、替代假說與 next-test 建議。`academicPromotionGate()` 負責阻止 unsupported claim 或 P0/P1 rebuttal 被靜默升級；`academicReviewArtifact()` 讓 live review 可重現、可 hash、可存證。
...
- `proResearchJob()` / `importProResearchReport()` / `proResearchPromotionGate()` 是 ChatGPT Pro research lane；它是 async job/import，不是同步 `/v1/responses` dropdown model。若 `chatgpt-pro-mcp` 可用，優先走 `submit_deep_research -> fetch_deep_research_result`；若不可用才手動執行/貼回。匯入時必須有 `research_ran_at` 或 MCP task metadata、`source_links`，且所有待 promotion claims 都有 claim-level evidence；否則 gate 會 fail-closed，unsupported claims 只能當 hypothesis。
- 大量 subagents 一律 schema 化、短輸出、可去重；不要讓自由長文淹沒主控上下文。
- `trading` profile 會搭配 `trading-training`：外部深度研究優先走 `chatgpt-pro-mcp` Pro research lane（只有 `research_mode.enabled=true` 才稱 confirmed Deep Research），GPT/Codex 主控 gate 與落地，MiniMax M3 扛策略候選與結果整理，Sonnet/Grok 做反方與快速副審，Opus 4.8 只審 top candidates / router / liquidation；所有 live_blocked、資金、訂單、槓桿或 stop-loss 變更仍必須走 Claude `trade-review`，且外部模型不得直接執行交易工具。

### ChatGPT Pro Research MCP lane

若已安裝 `chatgpt-pro-mcp`，主控代理可把 ChatGPT Pro 當成「訂閱型研究副腦」而不把人類拖進複製貼上流程：

1. Codex 產生有邊界的 `ProResearchJobV1`：問題、禁止事項、primary-source 要求、輸出欄位、日期、privacy classification。
2. `chatgpt-pro-mcp` 送入 ChatGPT Pro Web/App UI；`RETURN_AFTER_SUBMIT=1` 時初始 submit 只回 `running`。
3. open-ultrawork subagent 以 `fetch_deep_research_result({ task_id, wait_seconds })` 等待並 reattach，同步回收 Markdown/source links/claim ledger。
4. Codex 對來源做 cross-check，標記 unsupported / stale / hallucinated claims。
5. 只有通過驗證的 claims 才能進入 artifact，例如 `AiMarketSnapshotV1`、`AiRouterDecisionV1`、套利 cashflow spec 或 strategy research note。

手動在 ChatGPT Pro / Deep Research 執行並貼回 Markdown / PDF / source links 是 fallback，不是主路線。若 `authority.research_mode.enabled=false`，輸出仍可作 Pro Web research memo，但不得寫成 confirmed Deep Research。

硬邊界：

- Pro Research 不進 hot path，不可用於實時下單、撤單、槓桿、停損、倉位大小或 API key 操作。
- 不把 secrets、交易所 key、白名單、私有帳務明細丟給 ChatGPT UI。
- UI bridge 不得 auto-login、處理付款/terms/settings/history、暴露 cookies/profile 或提供 raw browser tool。
...
### 2026-06-01 OpenClaw / ULTRAWORK-M 健檢蒸餾

本輪 OpenClaw 18789 健檢驗證了一個可複用的 M 級 profile：
...
- **GPT-5.5 primary / hidden ChatGPT Pro compat alias**：`gpt-5.5` 是 Codex App 內的顧問 route，透過 ChatGPT subscription passthrough；舊 `chatgpt-pro-consult` 只作 hidden/deprecated alias。raw localhost GPT passthrough 會因缺 active Codex ChatGPT Authorization headers 而 401，這是正確 fail-closed，不是模型壞。要驗 GPT 路徑，用 Codex-native `codex exec --model gpt-5.5` 或 Codex App 內建路徑；alias 只作舊 thread 相容性檢查。
...
4. **profile 是分層，不是硬改 runtime**：例如 OpenClaw 18789 內部可維持 MiniMax dashboard；`gpt-5.5` fast consult 與 `chatgpt-pro-mcp` research lane 是外層 Codex 顧問/研究 lane，不應為了名稱一致去改 live gateway model。
...
|---|---|---|---|---|
| 前端 UI 風格摸索 | `gpt-5.5` fast consult 或 Codex 主控定義產品目標 | Minimax 大量產生 moodboard、版型、文案、元件變體；可加 Grok 查趨勢/反例 | Opus/GPT heavy 收斂成 2-3 個設計方向 | 截圖/原型、使用者偏好、品牌一致性、不要直接讓 subagent 改全站 |
| 前端實作 / component build | Codex/`gpt-5.5` fast consult 落地 | Minimax 產生候選元件、測試樣例、可訪問性 checklist | Opus 或 GPT heavy review 架構/狀態流 | browser smoke、lint、視覺 diff、互動流程 |
...
| AI 環境鋪設 / gateway / OpenClaw | `gpt-5.5` fast consult/Codex 主控；GitNexus 只在專案地圖需求時用 | Minimax 做 config inventory、log classification、runbook 草稿 | Opus 4.8 驗算拓樸、權限、成本、回滾 | health endpoints、LaunchAgent、process/port、secret redaction、rollback |
...
| 學術/研究級協作 | Codex 主控 packet 與驗證 | Minimax/Grok 做窄範圍 source/extraction/refutation | `chatgpt-pro-mcp` async Pro research lane 做 source-backed memo / claim/evidence/rebuttal；Opus 只審 unresolved high-risk | primary sources、claim ledger、unsupported 降級為 hypothesis、deterministic next tests |
...
建議分工：`T0-premium` = `fable-5`/`fable5` 做候選架構與最終 synthesis；`T1-copilot` = `gpt-5.5` fast consult（舊 `chatgpt-pro-consult` 只作 hidden alias）做 Codex-native critique/修補；`T1-research` = `chatgpt-pro-mcp` async Pro research lane 做 source-backed memo；`T2-judge` = `opus-4-8` 做保守反駁裁決；`T3-scout` = 便宜模型做窄輸出探索；`executor_host` = Codex App / Codex CLI / OpenClaw / sandbox 中被授權的一方。流程為 scout → GPT fast consult/Pro research MCP → Fable5 → Opus 4.8 judge gate → evidence 驗證 → executor_host 執行。routine coding、簡單 review、查詢、無 Done condition 或 latency-sensitive 任務不得使用。

### 模型性格速記

- **GPT-5.5 fast consult / ChatGPT Pro Research MCP**：`gpt-5.5` 適合 Codex-native planning、critique、工具執行語義與快速 review；`chatgpt-pro-mcp` 適合長時間 source-backed memo、claim/evidence/rebuttal 學術協作。兩者都不適合做大量廉價枚舉，也不能無本地 trace 宣稱執行 side effect。
...
## Tatwo Ultrawork M2 工作流邊界

Tatwo Ultrawork M2 可視為「工作流與主機接線可用版」；M3 UI 仍是獨立里程碑。M2 能拿來開工跑專案，但每次專案 run 都必須重新產生本次任務的收據，不能拿舊聊天或模型自評當完成證明。

M2 專案 run 的最低證據：

- **工作模式收據**：記錄 S/M/L/XL、預算、輪次、subagent 上限、停止條件。
- **入口與影響範圍收據**：M/L/XL 啟動前固定做一次 GitNexus project-map / impact-range；S 小修可跳過。若 GitNexus 尚未索引目標 repo，必須明確標記為 `project_map_fallback`，用本地 deterministic 掃描補上，並列為待補風險；不得靜默跳過。
- **分工收據**：主控、scout、reviewer、judge、executor 分開標示；MiniMax/Grok 等可給候選與反例，不可當最終驗收者。
- **驗證收據**：測試、smoke、lint、diff review、same-thread continuity 或對應情境的可重現證據。
- **回滾收據**：高風險修改要有備份、回滾路徑與失敗時停止條件。
- **MCP / plugin 收據**：需要 GitNexus、ChatGPT Pro MCP、gateway 或其他插件時，先證明可用或明確降級。

UI / UJ 相關任務的驗收要額外嚴格：build pass、Swift tests pass、模型說「看起來可以」都不算 UI pass。必須至少有截圖或可互動 smoke、具體原始問題重現步驟、修後對照、視覺或互動檢查結果；主控 GPT 不可單獨批准自己的 UI 修正，至少要有獨立 reviewer 或人類確認。若沒有視覺證據，結論只能寫「工程測試通過，UI 尚未驗收」。

ChatGPT Pro MCP 在 M2 中是 bounded Research / Plan / Review lane，不是 dropdown 模型，也不是寫檔執行者。它的輸出要進 claim/evidence ledger，Codex 再用 repo diff、測試或 primary source 驗證後才能 promotion。

若 Codex App / CLI 在長任務中斷，M2 專案應留下 public-safe recovery breadcrumb：只放目標、模式、收據摘要、可重跑命令與下一步；不放 token、raw log、完整聊天、私人 thread id、私密路徑或截圖原檔。外部 CLI / OpenClaw 接手時必須先讀 breadcrumb，再重跑 deterministic gate；不能從模型文字推斷已完成。

## GitNexus / GBrain 使用

- GitNexus 是被動專案地圖工具；只有使用者明示 `/gitnexus`、`用 GitNexus`、`專案地圖`、`入口`、`影響範圍`，或本 skill 已判定任務有明確架構/入口/資料流/影響範圍需求（clear project-map need）時才使用。Tatwo Ultrawork M2 的例外是：**M/L/XL workflow 固定需要一次 project-map / impact-range 收據**；S 小修可跳過。不要把 GitNexus 變成每次 grep / file read 前的強制 hook。
...
- UI / UJ 修改若沒有截圖、互動 smoke 或人類確認，不能宣稱 UI 已通過；只能宣稱工程層驗證通過。
