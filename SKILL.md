...
- **ChatGPT Pro lane**：日常協作優先走 Codex App / `model_gateway` 的 `chatgpt-pro-consult` 同 thread 顧問模型；Pro 做 research / planning / review，Codex 做 grounded execution。ChatGPT Web / Deep Research 只作長研究分流。
...
`RPO` 在本 skill 中指 **Research / Plan / Operate**：ChatGPT Pro 負責研究與規劃，Codex 負責可驗證執行。若工作從 Codex 開始，主線是 **Codex → `chatgpt-pro-consult`**；若工作從 ChatGPT 開始，才使用 **ChatGPT Pro → Codex MCP Hub** 前門。不要把 ChatGPT Web 假裝成背景 MCP server。
...
1. **Codex → ChatGPT Pro Consult（Codex 主控，日常主線）**
   ...
   - `copilot` / `T1-copilot` tier 預設使用 `codex:chatgpt-pro-consult`；外部模型只給 critique / plan / risk，不直接寫檔或執行 shell。
...
3. **ChatGPT Pro / Deep Research 手動研究分流**
   - 需要長篇外部研究時，Codex 產生 bounded prompt，使用者在 ChatGPT Pro / Deep Research 手動執行，再把含來源的 report 匯回；Codex 只採納經 primary-source cross-check 的 claims。
...
- Codex 先用 `academicContinuityPacket()` 建立 public-safe packet：問題、限制、決策、claims、open questions、驗證命令、artifact refs、content hash。
- `proAcademicPrompt()` 把 packet 轉成 Pro reviewer prompt，要求 Pro 做來源/證據核查、反駁、替代假說與 deterministic Codex next tests。
- `proAcademicReview()` 可直接走 `chatgpt-pro-consult` 執行一次 schema 化 Pro 審稿；外部模型仍不能寫檔或跑 shell。
...
硬邊界：不把 secrets、私有帳務、完整 logs、私人 thread id、原始 prompt dump、token 或本機絕對路徑放進 ChatGPT Pro / MCP public connector；ChatGPT Pro 不直接寫檔、跑 shell、部署、下單或修改風險參數。Codex 是唯一 side-effect executor。
...
關鍵認知：**主控不能 fan-out ≠ 做不到**，而是缺一支編排腳本。補上腳本，Codex CLI 用 `node` 執行即可真的並行 subagents；主控（Codex）仍是唯一工具執行者與最終驗證者，腳本是它的工具。
...
const packet = academicContinuityPacket({ task, claims, verificationCommands, artifactRefs });
...
```text
ULTRAWORK-M: gpt5.5-primary / opus4.8-review / minimax-m3-sub / codex-writes-only
```
...
- **Codex writes-only**：不論 subagent 來自哪個模型，只有主控 Codex 執行寫檔、shell、測試、部署、回滾；外部模型只提供 evidence / intent / critique。
...
4. **profile 是分層，不是硬改 runtime**：例如 OpenClaw 18789 內部可維持 MiniMax dashboard；ChatGPT Pro Consult / GPT-5.5 是外層 Codex 顧問主控 lane，不應為了名稱一致去改 live gateway model。
...
建議分工：`T0-premium` = `fable-5`/`fable5` 做候選架構與最終 synthesis；`T1-copilot` = `chatgpt-pro-consult` 做 Codex-native critique/修補；`T2-judge` = `opus-4-8` 做保守反駁裁決；`T3-scout` = 便宜模型做窄輸出探索；`executor` = Codex，唯一能寫檔、跑 shell、部署、回滾的執行者。流程為 scout → ChatGPT Pro Consult → Fable5 → Opus 4.8 judge gate → Codex evidence 驗證 → Codex 執行。routine coding、簡單 review、查詢、無 Done condition 或 latency-sensitive 任務不得使用。
